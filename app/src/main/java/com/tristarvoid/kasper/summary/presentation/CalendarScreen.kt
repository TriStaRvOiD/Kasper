...
package com.tristarvoid.kasper.summary.presentation
...
import com.tristarvoid.kasper.core.presentation.components.CustomAppBar
import com.tristarvoid.kasper.core.presentation.utils.calculateCalorieValue
import com.tristarvoid.kasper.core.presentation.utils.displayText
...
fun Calendar(
    ...
    val dataList = stepsState.dailyStepData
    ...
    val startDate = remember {
        if (!dataListIsEmpty)
            LocalDate.ofEpochDay(dataList[0].epochDay)
        else
            currentDate
    }
...
fun DayContent(
    ...
    Column(
        ...
        for (stepsData in stepsState.dailyStepData) {
            if (stepsData.epochDay == epochDay) {
                Text(text = "Step count : ${stepsData.totalStepCount}")
                Spacer(modifier = Modifier.height(10.dp))
//                Text(text = "Goal = ${stepsState.goal}")
//                Spacer(modifier = Modifier.height(10.dp))
//                Text(
//                    text = "Steps Remaining = ${
//                        if (stepsState.goal > stepsData.totalStepCount)
//                            stepsState.goal - stepsData.totalStepCount
//                        else
//                            0
//                    }"
//                )
                Text(text = "Steps indoors : ${stepsData.indoorStepCount}")
                Spacer(modifier = Modifier.height(10.dp))
                Text(text = "Steps outdoors : ${stepsData.outdoorStepCount}")
                Spacer(modifier = Modifier.height(10.dp))
    ...
    Box(
        ...
        Column(
            ...
            Text(
                ...
                fontSize = 14.sp,
                color = if (isSelected && MaterialTheme.colorScheme.isDark()) Color.Yellow else if (isSelected && !MaterialTheme.colorScheme.isDark()) Color.Cyan else LocalContentColor.current,
            )
        }
        if (isSelected) {
            Box(
