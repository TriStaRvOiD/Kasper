...
val Context.onboardDataStore: DataStore<Preferences> by preferencesDataStore(name = "on_boarding_pref")
...
class OnBoardDataStoreRepository(context: Context) {
    ...
    private val onboardDataStore = context.onboardDataStore

    suspend fun saveOnBoardingState(completed: Boolean) {
        onboardDataStore.edit { preferences ->
            preferences[PreferencesKey.onBoardingKey] = completed
        }
    }

    fun readOnBoardingState(): Flow<Boolean> {
        return onboardDataStore.data
            .catch { exception ->
